...
if [ $BATCH == True ]; then
    mkShapesMulti.py --pycfg=configuration.py --tag=$YEAR$TAG --sigset=$SIGSET --treeName=Events --outputDir=./Shapes/$YEAR/$TAG/$SPLIT --doBatch=True --batchQueue=$QUEUE --batchSplit=$SPLIT  --nuisancesFile=nuisances.py
else
