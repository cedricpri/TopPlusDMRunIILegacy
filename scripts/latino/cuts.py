...
if 'Test' in opt.tag:
    ...
    supercut = '((abs(Lepton_pdgId[0]) != 11 || ((Lepton_eta[0] + Electron_deltaEtaSC[Lepton_electronIdx[0]]) < -1.566 || ((Lepton_eta[0] + Electron_deltaEtaSC[Lepton_electronIdx[0]]) > -1.444 && (Lepton_eta[0] + Electron_deltaEtaSC[Lepton_electronIdx[0]]) < 1.444) || (Lepton_eta[0] + Electron_deltaEtaSC[Lepton_electronIdx[0]]) > 1.566)) && ((abs(Lepton_pdgId[1]) != 11 || ((Lepton_eta[1] + Electron_deltaEtaSC[Lepton_electronIdx[1]]) < -1.566 || ((Lepton_eta[1] + Electron_deltaEtaSC[Lepton_electronIdx[1]]) > -1.444 && (Lepton_eta[1] + Electron_deltaEtaSC[Lepton_electronIdx[1]]) < 1.444) || (Lepton_eta[1] + Electron_deltaEtaSC[Lepton_electronIdx[1]]) > 1.566)))) && nCleanJet > 0 && mll > 20'

    supercut = OC+' && '+supercut
    cuts['inclusiveCR_ll'] = supercut
    cuts['step1'] = '1'  
    cuts['step2'] = supercut + ' && (Lepton_pdgId[0] * Lepton_pdgId[1] == -11*13 || (mll < 76 || mll > 106))'
    cuts['step3'] = supercut + ' && (Lepton_pdgId[0] * Lepton_pdgId[1] == -11*13 || (mll < 76 || mll > 106)) && nbJet > 0'
    cuts['step4'] = supercut + ' && (Lepton_pdgId[0] * Lepton_pdgId[1] == -11*13 || (mll < 76 || mll > 106)) && nbJet > 0 && METcorrected_pt > 100.'
    cuts['step5'] = supercut + ' && (Lepton_pdgId[0] * Lepton_pdgId[1] == -11*13 || (mll < 76 || mll > 106)) && nbJet > 0 && METcorrected_pt > 100. && mt2ll > 80'
    ...
    for massPoint in signalMassPoints:
        if massPointInSignalSet(massPoint, opt.sigset):
            ...
            for model in ["ST", "TTbar"]:
                ...
                cuts[model + '_topCR_ll_BDT_' + training['shortName']] = { 'expr': supercut + ' && METcorrected_pt > 100. && (' + training['tag'] + ' == 0)'}
